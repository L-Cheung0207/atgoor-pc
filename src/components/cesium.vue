<template>
  <div>
    ...
    <div id="cesiumContainer" class="touchAll"></div>
  ...
  methods:{
    ...
    checkCesium(){
      ...
      if(this.addedCesiumCss && this.addedCesiumVectorTileScript  && this.addedCesiumScript){
        Cesium.Ion.defaultAccessToken = this.cesiumJsonData["accessToken"];
        ...
        self.VectorTileImageryProvider = Cesium.VectorTileImageryProvider;
        ...
        //   new Cesium.IonImageryProvider({ assetId: 695355 })
        // );
        // //蘇州 底圖
        // let imageryLayerSuzhou = self.viewer.imageryLayers.addImageryProvider(
        //   new Cesium.IonImageryProvider({ assetId: 712032 })
        // );
        ...
        this.viewer.camera.flyTo({
          destination : Cesium.Cartesian3.fromDegrees(114.1734595,22.301333, 10000000),
          duration : 0.1,
        });

        //設定網格
        // var primitives = {
        //   equator: this.parallel(0, Cesium.Color.BLUE),
        //   primeMeridian: this.meridian(0, Cesium.Color.BLUE),
        //   selectedPoint: {
        //     meridian: undefined,
        //     parallel: undefined,
        //     label: undefined,
        ...
        //     label: undefined,
        //   },
        //   // lowResolutionGrid: this.makeGrid(2, Cesium.Color.PALEGREEN, false),
        //   higherResolutionGrid: this.makeGrid(3, Cesium.Color.ALICEBLUE, false),
        // };
...
makeParallelsRecursive(
  ...
  return result;
    ...
    rotateEarth(angle){
      ...
      self.viewer.scene.camera.rotate(Cesium.Cartesian3.UNIT_Z, angle);
    },
    ...
    backToSearch(){
      ...
      this.resetSubCategoryData();
      this.viewer.camera.flyTo({
        destination : Cesium.Cartesian3.fromDegrees(114.1734595,22.301333, 50000000),
        duration : 2,
      });
    ...
    resetSubCategoryData(){
      for(var loop = 0 ; loop < this.subCategoryDataArray.length ; loop ++){
      ...
      for(var loop = 0 ; loop < this.isShowDataResultArray.length ; loop ++){
      ...
      if(!this.isDrawingData){
        ...
        if(store.state.selectingLocation == "saudi" && subCategoryData == "cityRoad" && this.isEnableSubCategoryData["cityRoad"]){
          ...
          if(this.dataProvider['saudiRoad'] == null){
            this.dataProvider['saudiRoad'] = new Cesium.IonImageryProvider({ assetId: 705964 });
          }
        ...
        if(store.state.selectingLocation == "saudi" && subCategoryData == "cityBuilding" && this.isEnableSubCategoryData["cityBuilding"]){
          ...
          if(this.dataProvider['saudiBuilding'] == null){
            this.dataProvider['saudiBuilding'] = new Cesium.IonImageryProvider({ assetId: 705962 });
          }
        ...
        if(store.state.selectingLocation == "hk" && subCategoryData == "cityRoad" && this.isEnableSubCategoryData["cityRoad"]){
          ...
          if(this.dataProvider['hkRoad'] == null){
            this.dataProvider['hkRoad'] = new Cesium.IonImageryProvider({ assetId: 711458 });
          }
        ...
        if(store.state.selectingLocation == "hk" && subCategoryData == "cityBuilding" && this.isEnableSubCategoryData["cityBuilding"]){
          ...
          if(this.dataProvider['hkBuilding'] == null){
            this.dataProvider['hkBuilding'] = new Cesium.IonImageryProvider({ assetId: 711457 });
          }
        ...
        if(store.state.selectingLocation == "suzhou" && subCategoryData == "cityRoad" && this.isEnableSubCategoryData["cityRoad"]){
          ...
          if(this.dataProvider['suzhouRoad'] == null){
            this.dataProvider['suzhouRoad'] = new Cesium.IonImageryProvider({ assetId: 712030 });
          }
        ...
        if(store.state.selectingLocation == "suzhou" && subCategoryData == "cityBuilding" && this.isEnableSubCategoryData["cityBuilding"]){
          ...
          if(this.dataProvider['suzhouBuilding'] == null){
            this.dataProvider['suzhouBuilding'] = new Cesium.IonImageryProvider({ assetId: 712029 });
          }
  ...
  watch:{
    ...
    checkLeftMenuSelectingArea(area){
      if(area != ""){
        ...
        this.viewer.camera.flyTo({
          destination : Cesium.Cartesian3.fromDegrees(areaData.longitude,areaData.latitude, areaData.height),
          duration : 2,
        });
      }
      else{
    ...
    checkCesiumBaseMapData(data){
      if(data != null){
        if(data != ""){
          ...
          this.baseMapLayerProvider = new Cesium.IonImageryProvider({ assetId: data });
    ...
    checkCesiumDataLayerData(data){
      if(data != null){
        if(data != ""){
          ...
          this.dataLayerProvider = new Cesium.IonImageryProvider({ assetId: data });
    ...
    //       destination : Cesium.Cartesian3.fromDegrees(114.1771163,22.3101859, 30000000),
    //       duration : 0.1,
    //     });
        
    //     setTimeout(function(){
    //       self.viewer.camera.flyTo({
    //       destination : Cesium.Cartesian3.fromDegrees(114.1771163,22.3101859, 1500000),
    //       duration : 0.1,
    //     });
    //     },500)
    ...
    //     setTimeout(function(){
    //       self.viewer.camera.flyTo({
    //       destination : Cesium.Cartesian3.fromDegrees(114.1771163,22.3101859, 150000),
    //       duration : 0.1,
    //     });
    //     },1000)
    ...
    //     setTimeout(function(){
    //       self.viewer.camera.flyTo({
    //       destination : Cesium.Cartesian3.fromDegrees(114.1771163,22.3101859, 5000),
    //       duration : 0.1,
    //     });
    //     },1500)
    ...
    //     self.viewer.camera.flyTo({
    //       destination : Cesium.Cartesian3.fromDegrees(46.5423449,24.5649316, 500000),
    //       duration : 0.1,
    //     });
        
    //     setTimeout(function(){
    //       self.viewer.camera.flyTo({
    //       destination : Cesium.Cartesian3.fromDegrees(46.5423449,24.5649316, 300000),
    //       duration : 0.1,
    //       });
    //     },500)
    ...
    //     setTimeout(function(){
    //       self.viewer.camera.flyTo({
    //       destination : Cesium.Cartesian3.fromDegrees(46.5423449,24.5649316, 150000),
    //       duration : 0.1,
    //       });
    //     },1000)
    ...
    //     setTimeout(function(){
    //       self.viewer.camera.flyTo({
    //       destination : Cesium.Cartesian3.fromDegrees(46.5423449,24.5649316, 6000),
    //       duration : 0.1,
    //       });
    //     },1500)
    ...
    //   else if(flag == "suzhou"){
    //     self.viewer.camera.flyTo({
    //       destination : Cesium.Cartesian3.fromDegrees(120.7377479,31.2837196, 500000),
    //       duration : 0.1,
    //     });
        
    //     setTimeout(function(){
    //       self.viewer.camera.flyTo({
    //       destination : Cesium.Cartesian3.fromDegrees(120.7377479,31.2837196, 300000),
    //       duration : 0.1,
    //       });
    //     },500)
    ...
    //     setTimeout(function(){
    //       self.viewer.camera.flyTo({
    //       destination : Cesium.Cartesian3.fromDegrees(120.7377479,31.2837196, 150000),
    //       duration : 0.1,
    //       });
    //     },1000)
    ...
    //     setTimeout(function(){
    //       self.viewer.camera.flyTo({
    //       destination : Cesium.Cartesian3.fromDegrees(120.7377479,31.2837196, 5000),
    //       duration : 0.1,
    //       });
    //     },1500)
